...
terraform {
  backend "s3" {
    ...
    key            = "3prodterraform.tfstate"  # State file name
...
module "nat" {
  ...
  subnet_id         = module.prod_network.pub_subnet_id_1
  vpc_id            = module.prod_network.vpc_id
...
module "eks_mgmt" {
  ...
  instance_name               = var.eks_instance_name
  associate_public_ip_address = var.associate_public_ip_address
  user_data                   = file("../2_prod_userdata_files_servers/1_eks.sh")
}
...
module "eks" {
  ...
  vpc_id = module.prod_network.vpc_id
  # subnet_id = [ module.nat.pvt_subnet_id , module.nat.pvt_subnet_id_1 ]
  subnet_id = [module.prod_network.pub_subnet_id_1, module.prod_network.pub_subnet_id_2]
  sg_ids    = module.sg.vpc_security_group_ids
